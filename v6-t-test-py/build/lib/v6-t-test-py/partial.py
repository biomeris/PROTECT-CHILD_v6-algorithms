...
def partial(
    ...
    If `group_col` is provided, the same statistics are computed separately for
    each level of `group_col` (two-sample setting), enabling a single global
    comparison across multiple sites without exchanging row-level data.
    ...
    group_col : str | None, optional
        Column defining the groups. If provided, results are returned per group
        level.
